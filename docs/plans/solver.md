...
- **P1.1 Macro intermediate retention** — reserve a small quota of move-aware
  intermediate states during macro expansion.
- **P1.2 Keeper-arrival beam** — controlled evaluation of keeper-arrival
  diversity as an explicit search feature.
- **P1.3 First-push walk cost** — add exact walk-to-support as a soft ordering
  term in first-push ranking.
- **P1.4 Reschedule-derived schedule trace** — export compact per-box phase
  diagnostics from repair telemetry.

### P2 work (not started)

- **P2.1 Reschedule-value predictor** — skip/light/full repair recommendation.
- **P2.2 Route diagnosis script** — `diagnose:solver-route` CLI tool.
- **P2.3 Engine modularization** — extract keeper-arrival/Pareto structures as
...
1. Solver output is untrusted input — independent replay verification must
   never be bypassed.
