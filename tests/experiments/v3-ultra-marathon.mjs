/**
 * Ultra marathon: converges to baseline (~830 moves) with standard rewrite,
 * then applies enhanced features: weighted heuristic, bidirectional, coarse
 * identity, offset passes, and targeted permutation on worst walk segments.
 */
...
// Phase 2: Converge to baseline with standard rewrite (3-4 passes)
...
for (let pass = 0; pass < 5; pass++) {
  ...
  if (delta >= 0) {
    console.log(`  Pass ${pass + 1}: no improvement (${passSol.moves} >= ${prevMoves})`);
    break;
  }
...
// Phase 3: Enhanced iterative rewrite with all features
const tiers = [
  // Tier 1: medium windows with offset + targeted
  { name: "medium-offset", hWeight: 2, bidir: true, coarse: false, mbWeight: 1.5,
    extraPasses: 1, targeted: 15,
    permVis: 400_000, permWin: [16, 32, 48], perPerm: 5000,
    winPush: [16, 32, 48], winVis: 50_000, winTotal: 300_000,
    moveVis: 150_000, movePush: [1, 2, 4, 8, 16], moveAttempts: 30,
    moveMin: 2, perMove: 5000, moveExtra: 8, maxVis: 800_000 },
  // Tier 2: medium-large with coarse + offset + targeted
  { name: "medium-large", hWeight: 3, bidir: true, coarse: true, mbWeight: 2.0,
    extraPasses: 2, targeted: 20,
    permVis: 600_000, permWin: [32, 48, 64], perPerm: 8000,
    winPush: [32, 48, 64], winVis: 60_000, winTotal: 400_000,
    moveVis: 200_000, movePush: [1, 2, 4, 8, 16], moveAttempts: 40,
    moveMin: 2, perMove: 6000, moveExtra: 10, maxVis: 1_200_000 },
  // Tier 3: large windows, coarse + offset + targeted
  { name: "large-targeted", hWeight: 3, bidir: true, coarse: true, mbWeight: 2.0,
    extraPasses: 2, targeted: 30,
    permVis: 1_000_000, permWin: [48, 64, 96], perPerm: 12000,
    winPush: [32, 48, 64, 96], winVis: 80_000, winTotal: 600_000,
    moveVis: 300_000, movePush: [1, 2, 4, 8, 16, 32], moveAttempts: 50,
    moveMin: 1, perMove: 8000, moveExtra: 12, maxVis: 1_800_000 },
  // Tier 4: mega with all features maxed
  { name: "mega-all", hWeight: 5, bidir: true, coarse: true, mbWeight: 2.5,
    extraPasses: 2, targeted: 40,
    permVis: 1_500_000, permWin: [64, 96, 128], perPerm: 18000,
