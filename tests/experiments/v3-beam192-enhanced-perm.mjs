/**
 * Enhanced permutation optimization on the converged beam-192 path.
 * Uses the new multi-size targeted permutation with re-ranking after improvements.
 * Very large permutation windows (up to 138 pushes = half of 276).
 *
 * Waits for /tmp/v3-beam192-converged.json to be available.
 */
...
const pathFile = "/tmp/v3-beam192-converged.json";
...
console.log(`Starting from converged beam-192: m=${sol.moves} p=${sol.pushes} w=${sol.moves - sol.pushes}`);
...
for (let round = 0; round < 60; round++) {
  ...
  const result = search({
    ...
    permutationWindowPushes: permHeavy ? [32, 48, 64, 96, 138] : [16, 32, 48, 64],
    ...
    permutationTargetedWindowPushes: permHeavy ? [48, 64, 96, 138] : [32, 48, 64, 96],
  ...
  if (roundSol.moves < bestEverMoves) {
    ...
    writeFileSync("/tmp/v3-beam192-enhanced-best.json", JSON.stringify(currentPath));
  }
