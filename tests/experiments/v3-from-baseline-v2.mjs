/**
 * v2: Uses latest engine with walk-aware move-bridge heuristic,
 * weighted random windows, BFS-MST, and all optimizations.
 * Loads saved baseline path and runs maximum-effort optimization.
 *
 * Usage: node --experimental-strip-types tests/experiments/v3-from-baseline-v2.mjs [path-file]
 */
...
const tiers = [
  // Tier 1: quick warmup
  { name: "warmup", hWeight: 3, bidir: true, coarse: false, mbWeight: 2.0,
    mst: true, extraPasses: 1, targeted: 10,
    bridgeBidir: true, bridgeExtra: 2, bridgeWt: 1.3,
    permVis: 300_000, permWin: [8, 16, 32], perPerm: 4000,
    winPush: [8, 16, 32], winVis: 40_000, winTotal: 300_000,
    moveVis: 200_000, movePush: [1, 2, 4, 8], moveAttempts: 30,
    moveMin: 2, perMove: 5000, moveExtra: 6, moveRandom: 60, maxVis: 800_000 },
  // Tier 2: medium with MST
  { name: "medium-mst", hWeight: 4, bidir: true, coarse: false, mbWeight: 2.0,
    mst: true, extraPasses: 1, targeted: 20,
    bridgeBidir: true, bridgeExtra: 2, bridgeWt: 1.3,
    permVis: 500_000, permWin: [16, 32, 48], perPerm: 6000,
    winPush: [16, 32, 48], winVis: 50_000, winTotal: 400_000,
    moveVis: 250_000, movePush: [1, 2, 4, 8, 16], moveAttempts: 40,
    moveMin: 2, perMove: 6000, moveExtra: 8, moveRandom: 100, maxVis: 1_200_000 },
  // Tier 3: large with coarse
  { name: "large-mst", hWeight: 5, bidir: true, coarse: true, mbWeight: 2.5,
    mst: true, extraPasses: 2, targeted: 30,
    ...
    moveVis: 350_000, movePush: [1, 2, 4, 8, 16, 32], moveAttempts: 50,
    moveMin: 1, perMove: 8000, moveExtra: 10, moveRandom: 140, maxVis: 1_500_000 },
  // Tier 4: mega
  { name: "mega-mst", hWeight: 6, bidir: true, coarse: true, mbWeight: 2.5,
    ...
    moveVis: 450_000, movePush: [1, 2, 4, 8, 16, 32, 48], moveAttempts: 60,
    moveMin: 1, perMove: 12000, moveExtra: 14, moveRandom: 180, maxVis: 2_500_000 },
  // Tier 5: ultra
  { name: "ultra-mst", hWeight: 8, bidir: true, coarse: true, mbWeight: 3.0,
    mst: true, extraPasses: 3, targeted: 60,
    bridgeBidir: true, bridgeExtra: 4, bridgeWt: 1.5,
    permVis: 1_500_000, permWin: [64, 96, 128], perPerm: 20000,
    winPush: [64, 96, 128], winVis: 150_000, winTotal: 1_500_000,
    moveVis: 600_000, movePush: [1, 2, 4, 8, 16, 32, 48], moveAttempts: 80,
    moveMin: 1, perMove: 15000, moveExtra: 16, moveRandom: 250, maxVis: 3_000_000 },
];
...
for (let round = 0; round < MAX_ROUNDS; round++) {
  ...
  if (!result.path) {
    ...
    staleCount++;
    if (staleCount >= 5 && tierIdx >= tiers.length - 1) {
  ...
  writeFileSync("/tmp/v3-best-path-v2.json", JSON.stringify(currentPath));
