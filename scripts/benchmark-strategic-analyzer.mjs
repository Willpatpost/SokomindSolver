...
const allowed = new Set(["ids", "budgets", "runs", "search-ms", "rewrite", "child", "execution", "inference-work", "task-macros", "seed-index", "schedule-choices"]);
...
const scheduleChoices = integer(args.get("schedule-choices") ?? 0, 8);
...
if (args.has("child")) {
  ...
  const preparation = search({algorithm: "analyze-puzzle", state,
    ...(budgets[0] ? {strategicAnalysis: {maxMs: budgets[0], inferenceWork, scheduleChoices}} : {})});
  ...
  console.log(JSON.stringify({id: ids[0], analysisBudgetMs: budgets[0], searchBudgetMs: searchMs || null,
    execution, inferenceWork, scheduleChoices, taskMacros, seedIndex, planExecution: result.planDiagnostics?.strategicExecution,
  ...
  for (const id of ids) for (let run = 0; run < runs; run++) {
    ...
    for (const budget of run % 2 ? [...budgets].reverse() : budgets) {
      ...
      const child = spawnSync(process.execPath, ["--experimental-strip-types", fileURLToPath(import.meta.url),
        "--child", `--ids=${id}`, `--budgets=${budget}`, `--search-ms=${searchMs}`, `--execution=${variant}`, `--inference-work=${inferenceWork}`, `--schedule-choices=${scheduleChoices}`,
